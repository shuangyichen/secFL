...
func clientPhase1(serverAddress string, robust bool, logDegree uint64, scale float64, resiliency float64) (cpkString *C.char, shamirShareString *C.char, id int) {
	...
	//fmt.Println("bef defer")
	defer conn.Close()
	// fmt.Println("aft defer")
	///////// receive network parameters from server
	message, err := bufio.NewReader(conn).ReadString('\n')
	if err != nil {
		fmt.Println("Bufio Error:", err.Error())
		panic(err)
	}
	...
	pi := &party{}
	fmt.Println("start create the party object and setup keys")
	if robust {
...
func clientPhase2(inputs []float64, cpkString string, shamirShareString string, id int, serverAddress string, robust bool, logDegree uint64, scale float64, resiliency float64) {
	...
	for pieceCounter := range encInput {
		...
		// wg.Add(1)
		// encInput[pieceCounter] = ckks.NewCiphertext(params, 1, params.MaxLevel(), params.Scale())
		go func(Counter int, params *ckks.Parameters, encoder ckks.Encoder, encryptor ckks.Encryptor, pi *party) {
			// func(Counter int, params *ckks.Parameters, encoder ckks.Encoder, encryptor ckks.Encryptor, pi *party) {
			// defer wg.Done()
			mutex.Lock()
			encInput[Counter] = ckks.NewCiphertext(params, 1, params.MaxLevel(), params.Scale())
			pt := ckks.NewPlaintext(params, params.MaxLevel(), params.Scale())
			encoder.Encode(pt, pi.input[Counter], params.Slots())
			encryptor.Encrypt(pt, encInput[Counter])
			mutex.Unlock()
			wg.Done()
			// fmt.Printf("Counter = %v\n", Counter)
		}(pieceCounter, params, encoder, encryptor, pi)
